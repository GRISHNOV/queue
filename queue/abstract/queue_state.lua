...
local function create_state_fiber(tubes)
    ...
    fiber.create(function()
        ...
        while true do
            if current == queue_state.states.WAITING then
                local rc = pcall(box.ctl.wait_rw, 0.01)
                if rc then
                    ...
                    for name, tube in pairs(tubes) do
            ...
            elseif current == queue_state.states.RUNNING then
                local rc = pcall(box.ctl.wait_ro, 0.01)
                if rc then
...
-- Show current state in human readble format.
